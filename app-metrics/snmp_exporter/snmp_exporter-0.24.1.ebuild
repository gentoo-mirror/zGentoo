...
# go mod vendor && mkdir snmp_exporter-`git describe --tags | sed -E "s/v([0-9.]+)/\1/g"` && mv vendor snmp_exporter-`git describe --tags | sed -E "s/v([0-9.]+)/\1/g"`/vendor && \
# tar -c -I 'xz -9 -T0' -f vendor-snmp_exporter-`git describe --tags | sed -E "s/v([0-9.]+)/\1/g"`.tar.xz snmp_exporter-`git describe --tags | sed -E "s/v([0-9.]+)/\1/g"`/vendor
SRC_URI="
