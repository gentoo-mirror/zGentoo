...
SRC_URI+=" https://github.com/influxdata/ui/releases/download/OSS-${PV}/build.tar.gz -> ${P}-assets.tar.gz"

## how to generate deps:
# >> git clone https://github.com/influxdata/influxdb -b v2.7.8 /tmp/influxdb
# >> cd /tmp/influxdb && version=`git describe --tags | sed -E "s/v([0-9.]+)/\1/g"`;
# >> GOMODCACHE="${PWD}"/go-mod go mod download -modcacherw -x && \
# >> cd cd go-mod/github.com/influxdata/pkg-config@v0.2.11 && \
# >> GOMODCACHE="${PWD}"/go-mod go mod download -modcacherw -x && \
# >> tar -caf influxdb-${version}-deps.tar.xz go-mod

SRC_URI+=" https://vendors.simple-co.de/${PN}/${P}-deps.tar.xz"
