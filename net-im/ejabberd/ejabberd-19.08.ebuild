...
SRC_URI="https://www.process-one.net/downloads/${PN}/${PV}/${P}.tgz -> ${P}.tar.gz"

LICENSE="GPL-2"
SLOT="0"
KEYWORDS="~amd64 ~x86"
REQUIRED_USE="mssql? ( odbc )"
IUSE="captcha debug elixir full-xml hipe ldap mssql mysql nls odbc pam postgres redis riak roster-gw sip sqlite stun zlib"

RESTRICT="test"

CDEPEND="
	>=dev-erlang/cache_tab-1.0.18
	>=dev-erlang/eimp-1.0.10
	>=dev-erlang/fast_tls-1.1.0
	>=dev-erlang/fast_xml-1.1.35
	>=dev-erlang/fast_yaml-1.0.18
	>=dev-erlang/jiffy-0.14.8
	>=dev-erlang/jose-1.9.0
	>=dev-erlang/lager-3.6.5
	>=dev-erlang/luerl-0.3
	>=dev-erlang/mqtree-1.0.2
	>=dev-erlang/p1_oauth2-0.6.4
	>=dev-erlang/p1_utils-1.0.14
	>=dev-erlang/pkix-1.0.1
	>=dev-erlang/stringprep-1.0.15
	>=dev-erlang/xmpp-1.3.2
	>=dev-lang/erlang-21.2.7[hipe?,odbc?,ssl]
...
src_configure() {
	econf \
		...
		$(use_enable mssql) \
		$(use_enable mysql) \
		$(use_enable mysql new-sql-schema) \
		$(use_enable odbc) \
		$(use_enable pam) \
